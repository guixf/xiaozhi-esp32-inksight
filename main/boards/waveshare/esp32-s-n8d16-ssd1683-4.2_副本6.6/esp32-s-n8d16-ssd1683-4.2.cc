...
private:
    ...
    void HandleNetworkEventCallback(NetworkEvent event, const std::string& data = "") {
        ...
        ESP_LOGI(TAG, "[RUNTIME] HandleNetworkEventCallback: event=%d, in_deep_sleep_=%d", (int)event, in_deep_sleep_);
        
        switch (event) {
            case NetworkEvent::Connected:
                wifi_connected_ = true;
                display_->showAiChatStatus("IDLE", "WiFi OK");
                ESP_LOGI(TAG, "[RUNTIME] Network connected event received, in_deep_sleep_=%d", in_deep_sleep_);
                
                // 检查是否是唤醒状态
                if (in_deep_sleep_) {
                    // 唤醒状态：WiFi连接后立即启动对话
                    ESP_LOGI(TAG, "[RUNTIME] Wake from deep sleep, WiFi connected, starting chat NOW");
...
public:
    Esp32SN8d16Ssd168342()
        ...
        switch(wakeup_cause) {
            ...
            case ESP_SLEEP_WAKEUP_EXT0: wakeup_name = "EXT0"; break;
            case ESP_SLEEP_WAKEUP_EXT1: wakeup_name = "EXT1"; break;
            case ESP_SLEEP_WAKEUP_TIMER: wakeup_name = "TIMER"; break;
            case ESP_SLEEP_WAKEUP_GPIO: wakeup_name = "GPIO"; break;
            default: wakeup_name = "OTHER"; break;
        }
        ESP_LOGI(TAG, "[RUNTIME] Wakeup cause: %d (%s)", wakeup_cause, wakeup_name);
        
        // 使用局部变量保存唤醒状态，避免被过早修改
        bool is_wake_from_deep_sleep = (wakeup_cause == ESP_SLEEP_WAKEUP_EXT0 || 
                                        wakeup_cause == ESP_SLEEP_WAKEUP_EXT1 || 
                                        wakeup_cause == ESP_SLEEP_WAKEUP_TIMER);
        
        if (is_wake_from_deep_sleep) {
            in_deep_sleep_ = true;
            ESP_LOGI(TAG, "[RUNTIME] Woke from deep sleep");
        } else {
            in_deep_sleep_ = false;
            ESP_LOGI(TAG, "[RUNTIME] First boot");
        }
        ...
        // 根据唤醒原因执行不同逻辑
        if (is_wake_from_deep_sleep) {
            ESP_LOGI(TAG, "[RUNTIME] Woke from deep sleep, calling WakeFromDeepSleep...");
            wakeup_start_chat_ = true;  // 设置标志，表示唤醒后需要启动对话
            WakeFromDeepSleep();
        } else {
        ...
        Application::GetInstance().Schedule([this, is_wake_from_deep_sleep]() {
            ESP_LOGI(TAG, "[INKSIGHT] CheckAndInitializeInksight scheduled task running, is_wake=%d", is_wake_from_deep_sleep);
            CheckAndInitializeInksight(is_wake_from_deep_sleep);
        });
    }
    ...
    void EnterDeepSleep() {
        ...
        ESP_ERROR_CHECK(esp_sleep_enable_ext1_wakeup(1ULL << AI_BUTTON_GPIO, ESP_EXT1_WAKEUP_ANY_LOW)); // 低电平唤醒
        ESP_LOGI(TAG, "[RUNTIME] AI button wakeup enabled (GPIO%d, EXT1, ANY LOW)", AI_BUTTON_GPIO);
