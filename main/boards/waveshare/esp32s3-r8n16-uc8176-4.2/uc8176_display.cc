...
static const char* TAG = "Uc8176Display";
...
Uc8176Display::Uc8176Display(int width, int height, uc8176_spi_t spi_data)
...
void Uc8176Display::spi_gpio_init() {
...
void Uc8176Display::spi_port_init() {
...
void Uc8176Display::SPI_SendByte(uint8_t data) {
    esp_err_t ret;
    spi_transaction_t t;
    memset(&t, 0, sizeof(t));
    t.length = 8;
...
void Uc8176Display::EPD_SendData(uint8_t data) {
    set_dc_1();
    set_cs_0();
    SPI_SendByte(data);
    set_cs_1();
...
void Uc8176Display::EPD_SendCommand(uint8_t command) {
    set_dc_0();
    set_cs_0();
...
void Uc8176Display::writeBytes(const uint8_t* buffer, int len) {
    set_dc_1();
    set_cs_0();
    esp_err_t ret;
    spi_transaction_t t;
...
void Uc8176Display::EPD_TurnOnDisplay() {
    EPD_SendCommand(0x11);
    EPD_SendData(0x01);
    EPD_SendCommand(0x12);
    read_busy();
    EPD_SendCommand(0x02);  // Power Off after refresh
    read_busy();
}

void Uc8176Display::EPD_TurnOnDisplayFast() {
    EPD_SendCommand(0x11);
    EPD_SendData(0x01);
    EPD_SendCommand(0x12);
    read_busy();
...
void Uc8176Display::EPD_Init() {
    ...
    EPD_SendCommand(0x61);   // 分辨率设置
    EPD_SendData(0x01);
    EPD_SendData(0x90);
    EPD_SendData(0x01);
    EPD_SendData(0x2C);

    EPD_SendCommand(0x65);   // 起始源极/栅极
    EPD_SendData(0x00);
    EPD_SendData(0x00);
    EPD_SendData(0x00);
    EPD_SendData(0x00);
    ...
    EPD_SendData(0x00);   // HRST[8:3] = 0
    EPD_SendData(0x31);   // HRED[8:3] = 49 (400/8 - 1)
    EPD_SendData(0x00);   // VRST[8:0] 低字节 = 0
    EPD_SendData(0x00);   // VRST[8:0] 高字节 = 0
    EPD_SendData(0x2B);   // VRED[8:0] 低字节 = 299 & 0xFF
    EPD_SendData(0x01);   // VRED[8:0] 高字节 = (299 >> 8) & 0xFF
    EPD_SendData(0x01);   // PT_SCAN = 1
    // ===============================
    ...
    ESP_LOGI(TAG, "EPD initialized (UC8176, BWR color)");
}

void Uc8176Display::EPD_InitFast() {
    EPD_Reset();
    read_busy();
...
void Uc8176Display::EPD_SetFullWindow() {
/*    EPD_SendCommand(0x44);
    EPD_SendData(0);
    EPD_SendData((width_ - 1) / 8);
    EPD_SendCommand(0x45);
...
void Uc8176Display::EPD_Display(const uint8_t* image) {
    ...
    EPD_SendCommand(0x10);  // Write Black/White RAM for UC8176
    writeBytes(image, spi_data_.buffer_len);

    EPD_SendCommand(0x13);  // Write RED RAM for UC8176
    if (red_buf_) {
        writeBytes(red_buf_, spi_data_.buffer_len);
    } else {
        writeBytes(image, spi_data_.buffer_len);
    ...
    EPD_SendCommand(0x11);  // Data Entry Mode for UC8176
    EPD_SendData(0x01);     // data_flag = 1
    EPD_SendCommand(0x12);  // Display Refresh for UC8176
    read_busy();
...
void Uc8176Display::EPD_DisplayFast(const uint8_t* image) {
    EPD_SendCommand(0x02);  // Power Off
    read_busy();
    EPD_SendCommand(0x04);  // Power On
    vTaskDelay(pdMS_TO_TICKS(10));
...
void Uc8176Display::clearBuffer() {
    if (img_buf_) {
        memset(img_buf_, 0xFF, spi_data_.buffer_len);  // 全白 (0x24=0xFF)
    }
    if (red_buf_) {
        memset(red_buf_, 0x00, spi_data_.buffer_len);  // 全非红色 (0x26=0x00)
    }
}

void Uc8176Display::RequestDisplayUpdate() {
...
void Uc8176Display::ProcessDisplayUpdate() {
...
bool Uc8176Display::IsDisplayBusy() {
    return display_state_ != DisplayState::IDLE;
}

void Uc8176Display::EPD_DisplayPartial(const uint8_t* image, int x_start, int y_start, int x_end, int y_end) {
...
void Uc8176Display::TriggerImageUpdate() {
    if (external_image_callback_ && img_buf_) {
        external_image_callback_(img_buf_);
    }
}

void Uc8176Display::SetStatus(const char* status) {
...
void Uc8176Display::SetSuppressAutoImageFetch(bool suppress) {
    ESP_LOGI(TAG, "[INKSIGHT] SetSuppressAutoImageFetch: %d", suppress);
    suppress_auto_image_fetch_ = suppress;
}

void Uc8176Display::ShowNotification(const char* notification, int duration_ms) {
...
void Uc8176Display::InitializeStatusLed() {
...
void Uc8176Display::SetStatusLedMode(int mode) {
    // 如果模式相同，不需要重复设置
    if (led_mode_ == mode) {
        return;
    }
    ...
    switch (mode) {
        ...
        case LED_MODE_ACK:
            ...
            led_state_on_ = false;
            ESP_LOGI(TAG, "[LED] Mode: ACK (2 blinks)");
            break;
            
        case LED_MODE_CONNECTING:
            // connecting: 慢速闪烁 (200ms亮, 200ms灭)
            if (!led_blink_timer_) {
                esp_timer_create_args_t args = {};
                args.callback = &LedBlinkTimerCallback;
                args.arg = this;
            ...
            led_state_on_ = false;
            gpio_set_level(STATUS_LED_GPIO, 0);
            esp_timer_start_periodic(led_blink_timer_, 400000);  // 400ms周期(200ms on/off)
            ESP_LOGI(TAG, "[LED] Mode: CONNECTING (200ms on/off)");
            break;
            
        case LED_MODE_DOWNLOADING:
            // downloading: 快速闪3次 (150ms亮, 150ms灭)
            for (int i = 0; i < 3; i++) {
                gpio_set_level(STATUS_LED_GPIO, 0);
            ...
            led_state_on_ = false;
            ESP_LOGI(TAG, "[LED] Mode: DOWNLOADING (3 blinks)");
            break;
        ...
        case LED_MODE_SUCCESS:
            ...
            gpio_set_level(STATUS_LED_GPIO, 1);
            led_state_on_ = false;
            ESP_LOGI(TAG, "[LED] Mode: SUCCESS (1s on)");
            break;
            
        case LED_MODE_FAIL:
            // fail: 快速闪5次 (60ms亮, 60ms灭)
            for (int i = 0; i < 5; i++) {
                gpio_set_level(STATUS_LED_GPIO, 0);
            ...
            led_state_on_ = false;
            ESP_LOGI(TAG, "[LED] Mode: FAIL (5 blinks)");
            break;
            
        case LED_MODE_FAVORITE:
            // favorite: 常亮2秒后熄灭
            gpio_set_level(STATUS_LED_GPIO, 0);
            led_state_on_ = true;
            ...
            gpio_set_level(STATUS_LED_GPIO, 1);
            led_state_on_ = false;
            ESP_LOGI(TAG, "[LED] Mode: FAVORITE (2s on)");
            break;
        ...
        case LED_MODE_PORTAL:
            ...
            gpio_set_level(STATUS_LED_GPIO, 0);
            led_state_on_ = true;
            ESP_LOGI(TAG, "[LED] Mode: PORTAL (on)");
            break;
    }
}

void Uc8176Display::LedBlinkTimerCallback(void* arg) {
    Uc8176Display* self = static_cast<Uc8176Display*>(arg);
...
void Uc8176Display::drawText(const char* msg, int x, int y, int scale) {
                    ...
                    for (int dy = 0; dy < scale; dy++) {
                        for (int dx = 0; dx < scale; dx++) {
                            ...
                            if (px >= 0 && px < width_ && py >= 0 && py < height_) {
                                int bufIndex = py * rowBytes + px / 8;
                                int bitPos = px % 8;
                                // 绘制黑色：img_buf_=0, red_buf_=0
...
void Uc8176Display::drawPixel(int x, int y, bool black) {
    if (x < 0 || x >= width_ || y < 0 || y >= height_ || !img_buf_) return;
    int rowBytes = width_ / 8;
    int bufIndex = y * rowBytes + x / 8;
    int bitPos = x % 8;
    if (black) {
        // 绘制黑色：img_buf_=0, red_buf_=0
        img_buf_[bufIndex] &= ~(0x80 >> bitPos);
...
void Uc8176Display::fillRect(int x, int y, int w, int h) {
    if (!img_buf_) return;
    int rowBytes = width_ / 8;
...
int Uc8176Display::textWidth(int charCount, int scale) {
    return charCount * (5 * scale + scale) - scale;
}

const uint8_t* Uc8176Display::getGlyph(char c) {
...
const char* Uc8176Display::aiStatusTitle(const char* state) {
    if (!state) return "AI";
...
const char* Uc8176Display::aiStatusDetailFallback(const char* state) {
...
void Uc8176Display::drawHLine(int x0, int x1, int y) {
    if (x1 <= x0) return;
    fillRect(x0, y, x1 - x0, 1);
}

void Uc8176Display::drawWrappedAsciiText(const char* msg, int x, int y, int width, int scale, int maxLines) {
...
int Uc8176Display::computeMaxWrappedLines(int startY, int endY, int scale) {
...
void Uc8176Display::drawRobotIcon(int ox, int oy, int scale) {
...
void Uc8176Display::showSetupScreen(const char* apName, const char* regCode) {
    clearBuffer();
...
void Uc8176Display::showAiChatStatus(const char* state, const char* detail) {
...
void Uc8176Display::showVoiceChatScreen() {
    clearBuffer();
...
void Uc8176Display::showVoiceIndicator(bool footerCenter) {
    if (!img_buf_) return;
...
void Uc8176Display::hideVoiceIndicator() {
...
void Uc8176Display::showError(const char* msg) {
    clearBuffer();
...
void Uc8176Display::showDiagnostic(const char* line1, const char* line2, const char* line3, const char* line4) {
    clearBuffer();
...
void Uc8176Display::showModePreview(const char* modeName) {
    clearBuffer();
...
void Uc8176Display::showWifiConfigScreen(const char* ap_name, const char* web_url) {
    clearBuffer();
...
std::string Uc8176Display::GetMacAddress() {
    uint8_t mac[6];
...
void Uc8176Display::SaveDeviceToken(const char* token) {
...
void Uc8176Display::ClearDeviceToken() {
    nvs_handle_t nvs_handle;
    esp_err_t err = nvs_open("storage", NVS_READWRITE, &nvs_handle);
    if (err == ESP_OK) {
...
std::string Uc8176Display::LoadDeviceToken() {
    nvs_handle_t nvs_handle;
...
bool Uc8176Display::EnsureDeviceToken() {
...
void Uc8176Display::SetInksightConfig(const char* server) {
...
std::string Uc8176Display::GetRuntimeMode() {
    return runtime_mode_;
}

void Uc8176Display::SetAlwaysActive(bool active) {
    // 确保 runtime_mode_ 与 always_active_ 始终同步
    std::string expected_mode = active ? "active" : "interval";
    
    if (always_active_ != active) {
        always_active_ = active;
        runtime_mode_ = expected_mode;
        ESP_LOGI(TAG, "[RUNTIME] Always active mode: %s, runtime_mode set to: %s", 
                 active ? "ON" : "OFF", runtime_mode_.c_str());
    } else if (runtime_mode_ != expected_mode) {
        // 当 always_active 未变化但 runtime_mode 不一致时进行修复
        runtime_mode_ = expected_mode;
        ESP_LOGI(TAG, "[RUNTIME] Fixed runtime_mode to: %s (was empty or mismatched)", runtime_mode_.c_str());
    }
...
bool Uc8176Display::IsAlwaysActive() {
    return always_active_;
}

void Uc8176Display::LoadAlwaysActiveFromNVS() {
    nvs_handle_t nvs_handle;
    esp_err_t err = nvs_open("storage", NVS_READONLY, &nvs_handle);
    if (err == ESP_OK) {
        uint8_t val = 0;
...
void Uc8176Display::SetLiveModeTimerCallback(std::function<void(bool enable)> callback) {
    live_mode_timer_callback_ = callback;
    ESP_LOGI(TAG, "[LIVE MODE] Live mode timer callback set");
}

void Uc8176Display::PostRuntimeMode(const char* mode) {
...
void Uc8176Display::FetchInksightImage() {
    if (inksight_server_.empty()) {
    ...
    BaseType_t result = xTaskCreatePinnedToCore(&Uc8176Display::InksightFetchTask, "inksight_fetch", 12288, this, 5, &inksight_task_handle_, 0);
    if (result != pdPASS) {
...
void Uc8176Display::InksightFetchTask(void* param) {
    Uc8176Display* self = static_cast<Uc8176Display*>(param);
...
void Uc8176Display::DoFetchInksightImage() {
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
        ...
        if (content_len == color_buf_len) {
            ...
            // 初始化缓冲区
            // 根据正确的颜色逻辑：
            // 白色：0x24=0xFF + 0x26=0x00 → img_buf_=1, red_buf_=0
            // 红色：0x24=0x00 + 0x26=0xFF → img_buf_=0, red_buf_=1
            // 黑色：0x24=0x00 + 0x26=0x00 → img_buf_=0, red_buf_=0
            memset(img_buf_, 0x00, buf_len);  // 默认黑色
            memset(red_buf_, 0x00, buf_len);  // 默认非红色
            ...
            for (int y = 0; y < height_; y++) {
                for (int x = 0; x < width_; x++) {
                    ...
                    // 2bpp颜色编码：0x00=黑, 0x01=白, 0x02=保留, 0x03=红
                    if (color == 0x00) {
                        // 黑色：img_buf_=0, red_buf_=0
                        img_buf_[bufIndex] &= ~(0x80 >> bitPos);
                        red_buf_[bufIndex] &= ~(0x80 >> bitPos);
                    } else if (color == 0x01) {
                        // 白色：img_buf_=1, red_buf_=0
                        img_buf_[bufIndex] |= (0x80 >> bitPos);
                        red_buf_[bufIndex] &= ~(0x80 >> bitPos);
                    } else if (color == 0x02) {
                        // 保留/未使用，当作白色处理
            ...
            // 统计红色像素数量（red_buf_中1表示红色）
            int red_pixel_count = 0;
            for (int i = 0; i < buf_len; i++) {
                if (red_buf_[i] != 0x00) {
                    red_pixel_count += __builtin_popcount(red_buf_[i]);
                }
            }
...
bool Uc8176Display::readExact(esp_http_client_handle_t client, uint8_t* buf, int len, int timeout_ms) {
    int got = 0;
    uint64_t t0 = esp_timer_get_time();
    while (got < len) {
...
bool Uc8176Display::IsWifiConnected() {
    wifi_ap_record_t ap_info;
...
bool Uc8176Display::IsVoiceInactive() {
...
void Uc8176Display::SendHeartbeat(bool force) {
    if (inksight_server_.empty()) {
    ...
    BaseType_t result = xTaskCreatePinnedToCore(&Uc8176Display::HeartbeatTask, "heartbeat", 8192, params, 5, NULL, 0);
    if (result != pdPASS) {
...
void Uc8176Display::HeartbeatTask(void* param) {
...
void Uc8176Display::DoSendHeartbeat(bool force) {
    if (inksight_server_.empty()) {
...
void Uc8176Display::CheckPendingRefresh() {
    if (inksight_server_.empty()) {
    ...
    BaseType_t result = xTaskCreatePinnedToCore(&Uc8176Display::PendingRefreshTask, "pending_refresh", 8192, this, 5, NULL, 0);
    if (result != pdPASS) {
        ESP_LOGE(TAG, "[INKSIGHT] Failed to create pending refresh task");
    }
}

void Uc8176Display::PendingRefreshTask(void* param) {
    Uc8176Display* self = static_cast<Uc8176Display*>(param);
    self->DoCheckPendingRefresh();
    vTaskDelete(NULL);
}

void Uc8176Display::DoCheckPendingRefresh() {
    if (inksight_server_.empty()) {
...
bool Uc8176Display::HttpGetState(const char* url, char** out_buffer, int* out_len) {
...
void Uc8176Display::FetchFocusListening() {
    if (inksight_server_.empty()) {
