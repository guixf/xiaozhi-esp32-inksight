...
void Ssd1683Display::EPD_Init() {
    ...
    // 发送 LUT 数据（关键：三色屏必须配置完整 LUT 才能正确显示红色）
    EPD_SendCommand(0x2C);  // VCOM DC Setting
    EPD_SendData(0x00);
    
    EPD_SendCommand(0x32);  // LUT FOR VCOM (完整的 9 组 LUT，共 270 字节)
    writeBytes(LUT_VCOM_DC, sizeof(LUT_VCOM_DC));  // VCOM DC
    writeBytes(LUT_WW, sizeof(LUT_WW));            // 白色→白色
    writeBytes(LUT_BW, sizeof(LUT_BW));            // 黑色→白色
    writeBytes(LUT_WB, sizeof(LUT_WB));            // 白色→黑色
    writeBytes(LUT_BB, sizeof(LUT_BB));            // 黑色→黑色
    writeBytes(LUT_WR, sizeof(LUT_WR));            // 白色→红色 (三色屏专用)
    writeBytes(LUT_BR, sizeof(LUT_BR));            // 黑色→红色 (三色屏专用)
    writeBytes(LUT_RW, sizeof(LUT_RW));            // 红色→白色 (三色屏专用)
    writeBytes(LUT_RB, sizeof(LUT_RB));            // 红色→黑色 (三色屏专用)
