...
/*    
    // ========== 红色显示测试 ==========
    // 根据正确的颜色逻辑测试：
    // 0x24=0x00 + 0x26=0xFF = 红色
    // 0x24=0xFF + 0x26=0x00 = 白色
    // 0x24=0x00 + 0x26=0x00 = 黑色
    ESP_LOGI(TAG, "[TEST] Forcing full red screen");
    ...
    // 写入黑白RAM（0x00表示黑色像素位置）
    EPD_SendCommand(0x24);
    memset(img_buf_, 0x00, spi_data_.buffer_len);  // 全0
    writeBytes(img_buf_, spi_data_.buffer_len);
    
    // 写入红色RAM（0xFF表示红色像素位置）
    EPD_SendCommand(0x26);
    memset(red_buf_, 0xFF, spi_data_.buffer_len);  // 全FF = 红色
    ...
    // 等待5秒让用户观察
    ESP_LOGI(TAG, "[TEST] Red screen displayed, waiting 5 seconds...");
    ...
    ESP_LOGI(TAG, "[TEST] Red screen test completed");
    // ========== 测试结束 ==========
 */   
    // 初始化 GPIO2 状态指示灯
    InitializeStatusLed();
...
void Ssd1683Display::EPD_Init() {
    ...
    EPD_SendData(0x00);     //   Source output mode    //原值    EPD_SendData(0x40);
    EPD_SendData(0x00);

/*
    epdSendCommand(0x3F);  // Border Waveform Control
    epdSendData(0x00);
*/


    EPD_SendCommand(0x22);    // Display Update Control 2
    EPD_SendCommand(0xF7);
...
//    EPD_SendData(0x00);
    ...
    EPD_SendCommand(0x44);  // Set RAM X address range
    EPD_SendData(0x00);
    EPD_SendData((width_ - 1) / 8);

    EPD_SendCommand(0x45);// Set RAM Y address range
    EPD_SendData(0x00);
    EPD_SendData(0x00);
    EPD_SendData((height_ - 1) & 0xFF);
    EPD_SendData(((height_ - 1) >> 8) & 0xFF);
