...
void WifiBoard::TryWifiConnect() {
    ...
    if (have_ssid) {
        ...
        // 尝试快速恢复 WiFi 连接（从RTC内存恢复配置）
        esp_err_t err = esp_wifi_restore();
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "WiFi config restored from RTC, attempting quick connect...");
            // 配置并启动WiFi为station模式
            wifi_config_t wifi_config = {};
            err = esp_wifi_get_config(WIFI_IF_STA, &wifi_config);
            if (err == ESP_OK && wifi_config.sta.ssid[0] != 0) {
                ESP_LOGI(TAG, "Found saved WiFi config, connecting to SSID: %s", wifi_config.sta.ssid);
                // 直接启动连接，WiFi驱动会自动尝试恢复会话
                WifiManager::GetInstance().StartStation();
            } else {
                ESP_LOGW(TAG, "No valid WiFi config found, doing full connect");
                WifiManager::GetInstance().StartStation();
            }
        } else {
            ESP_LOGI(TAG, "No WiFi config in RTC, doing full connect");
            WifiManager::GetInstance().StartStation();
        }
    } else {
...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
    ...
    if (network_event_callback_) {
        ESP_LOGI(TAG, "[DEBUG] Calling network_event_callback_, event=%d", (int)event);
        network_event_callback_(event, data);
    } else {
        ESP_LOGW(TAG, "[DEBUG] network_event_callback_ is null!");
    }
}
